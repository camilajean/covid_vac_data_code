...
# Generate formulas for fixed models
formulas_fixed <- setNames(
  lapply(dose_outcomes, function(outcome) {
    as.formula(paste(outcome, "~", paste(vars, collapse = " + ")))
  }),
  paste0(dose_outcomes, "_fixed")
)

# Generate formulas for random models
formulas_random <- setNames(
  lapply(dose_outcomes, function(outcome) {
    as.formula(paste(outcome, "~", paste(vars, collapse = " + "), "+ (1|RGN21CD)"))
  }),
  paste0(dose_outcomes, "_random")
)

# Combine all into one list
all_formulas <- c(formulas_fixed, formulas_random)

# Save as text to CSV
formula_text <- lapply(all_formulas, function(f) paste(deparse(f), collapse = " "))
write.csv(
  data.frame(model = names(formula_text), formula = unlist(formula_text)),
  "../results_tables/simple_model_formulas.csv",
  row.names = FALSE
)

# Optional: check in R
head(read.csv("../results_tables/model_formulas.csv"))
